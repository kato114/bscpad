...
export default function ProjectDetails() {
  ...
  const [toAddress, setToAddress] = useState(
    "0xfF09155510980ED164Ec2B2747C0CA5f067BDE34"
  );
  ...
  useEffect(() => {
    ...
    // getting allocations
    const allocationsQuery = ref(db, "allocations");
    onValue(allocationsQuery, (snapshot) => {
      const data = snapshot.val();
      setAllocations(data);
    });
    ...
    onValue(settingRef, (snapshot) => {
      const data = snapshot.val();
      setSettings(data);
      setToAddress(data.wallet_address);
      setLoading(true);
    });
  }, []);
                  ...
                  <div className="ms-0">
                    <div className="pp-card-top">
                      <div className="title-box">
                        <h2>
                          {Boolean(settings) ? settings.token_name : ""}
                          {Boolean(settings) && (
                            <>
                              {Boolean(settings.app_link) && (
                                <a href={settings.app_link} target="_blank">
                                  <i className="fas fa-globe-americas"></i>
                                </a>
                              )}
                              {Boolean(settings.twitter_link) && (
                                <a href={settings.twitter_link} target="_blank">
                                  <i className="fab fa-twitter"></i>
                                </a>
                              )}
                              {Boolean(settings.medium_link) && (
                                <a href={settings.medium_link} target="_blank">
                                  <i className="fab fa-medium-m"></i>
                                </a>
                              )}
                              {Boolean(settings.telegram_link) && (
                                <a
                    ...
                    <div>
                      <p className="mt-5 mb-0 text-white">
                        {Boolean(settings) ? settings.first_note : ""}
                      </p>
                      <p className="text-muted">
                        {Boolean(settings) ? settings.second_note : ""}
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="card-body px-3 py-2">
                      ...
                      <div className="pp-card-info">
                        <div className="d-flex justify-content-between w-100">
                          <div className="pp-card-col w-200px">
                            <p>
                              {Boolean(settings) ? settings.progress_title : ""}
                            </p>
                            <hr className="mb-2" />
                            <p className="mb-0">Show progress</p>
                            <div className="pp-card-progress">
                              ...
                              >
                                <div className="pp-card-progress-percent-bar"></div>
                              </div>
